locals {
  ...
  cloud_tasks_eu         = fileexists("${path.module}/config/cloudtask-eu.yaml") ? concat(yamldecode(templatefile("${path.module}/../config/cloudtask-eu.yaml", { region_eu = var.region_eu })),yamldecode(templatefile("${path.module}/config/cloudtask-eu.yaml", { region_eu = var.region_eu }))) : yamldecode(templatefile("${path.module}/../config/cloudtask-eu.yaml", { region_eu = var.region_eu }))
  pubsub_topics_eu        = fileexists("${path.module}/../config/topics-eu.yaml") ? concat(yamldecode(file("${path.module}/../config/topics-eu.yaml")),yamldecode(file("${path.module}/../config/topics-eu.yaml"))) : yamldecode(file("${path.module}/../config/topics-eu.yaml"))
...
locals {
  bank_bridge_k8s_service_accounts = [
    ...
    {
      k8s_service_account_name      = "zamp-bank-bridge-sync-merchant-transactions-job"
      k8s_service_account_namespace = "zamp"
    },
    {
  ...
  bank_bridge_pubsub_topics = [
    ...
    bank_bridge_pubsub_topics_eu = [
    {
      name                    = "payout-transaction-eu"
      dlq_enabled             = true
      ack_deadline_seconds    = 600
      enable_message_ordering = true
      enable_exactly_once_delivery = true
      retry_enabled                = false 
      subscriptions           = ["payout-transaction-eu"]
    },
    {
      name                         = "bank-bridge-bank-statements-eu"
      dlq_enabled                  = true
      ack_deadline_seconds         = 600
      enable_message_ordering      = true
      enable_exactly_once_delivery = true
      retry_enabled                = false 
      subscriptions                = ["bank-bridge-bank-statements-eu"]
    },
    {
      name                         = "bank-bridge-payout-transactions-ops-eu"
      dlq_enabled                  = true
      ack_deadline_seconds         = 600
      enable_message_ordering      = true
      enable_exactly_once_delivery = true
      retry_enabled                = false 
      subscriptions                = ["bank-bridge-payout-transactions-ops-eu"]
    }         
  ]
