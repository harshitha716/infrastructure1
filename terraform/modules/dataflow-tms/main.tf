...
resource "google_service_account" "service_account" {
  ...
  display_name = "${var.name}-service-account-tms"
}
...
module "dataflow_role" {
  ...
  role_id              = "dataflow_streaming_job_role_tms"
  title                = "DataflowStreamingJobRole-tms"
  description          = "Custom role for dataflow streaming job tms"
...
resource "google_compute_firewall" "dataflow_worker" {
  name    = "${var.project_prefix}-streaming-dataflow-tms"
