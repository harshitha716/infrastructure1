...
resource "google_compute_url_map" "https" {
  ...
  dynamic "host_rule" {
    for_each = toset(var.static_hosting_domain)
...
resource "google_compute_managed_ssl_certificate" "https" {
  for_each = random_id.certificate 
...
resource "google_compute_target_https_proxy" "https" {
  ...
  ssl_certificates = [for cert in google_compute_managed_ssl_certificate.https : cert.self_link]
